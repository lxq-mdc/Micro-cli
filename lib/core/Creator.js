...
const { IsCover, SelectFramework, IsConfigEslint, IsConfigPrettier, IsConfigGitHooks, IsConfigTs } = require('../utils/prompts') //导入一些选项
    ...
    create() {
        ...
        async function ConfigEslint() {
            if (isEslint) {
                ...
                ReactPackage = extendPackage(typeof ReactPackage === 'string' ? ReactPackage : JSON.stringify(ReactPackage), ReactEslintDependencies)
                    //增加.eslintrc文件的目录
                let targetPath = path.resolve(`${process.cwd()+'/'+this.projectName}`, '.eslintrc')
        ...
        async function ConfigPrettier() {
            if (isPrettier) {
                ...
                ReactPackage = extendPackage(typeof ReactPackage === 'string' ? ReactPackage : JSON.stringify(ReactPackage), ReactPrettierDependencies)
                    //增加.prettierrc文件的目录
                let targetPath = path.resolve(`${process.cwd()+'/'+this.projectName}`, '.prettierrc')
        ...
        async function ConfigTs() {
            if (isTs) {
                ...
                let tsConfigPath = path.resolve(`${process.cwd()+'/'+this.projectName}`, 'tsconfig.json')
                    //增加tsconfig.node.json文件的目录路径
                let tsConfigNodePath = path.resolve(`${process.cwd()+'/'+this.projectName}`, 'tsconfig.node.json')
                    //增加vite.config.ts文件的目录路径
                let viteConfigTsPath = path.resolve(`${process.cwd()+'/'+this.projectName}`, 'vite.config.ts')
                ...
                let viteConfigJsPath = path.resolve(`${process.cwd()+'/'+this.projectName}`, 'vite.config.js')
        ...
        async function ConfigGitHooks() {
            if (isGitHooks) {
                ...
                let targetPath = path.resolve(`${process.cwd()+'/'+this.projectName}`, '.husky')
        ...
        await download(Repo, this.projectName, { clone: true }, async(err) => {
            ...
            await ConfigTs()
            await writeToFile(path.resolve(process.cwd(), this.projectName, 'package.json'), JSON.stringify(ReactPackage))
