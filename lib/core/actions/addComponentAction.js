...
const { successLogWithBg } = require('../../utils/logWithChalk');
const { isRoot, isTypescript } = require('../../utils/is');
const projectType = require('../../utils/projectType');
...
const addComponentAction = async (componentName, options, pathName) => {
  ...
  switch (type) {
    ...
    case 'React': {
      ...
      const css = await complie('react-style.ejs', { name: componentName });
      await writeToFile(dirName + `/index.${isTs ? 'tsx' : 'jsx'}`, xml);
      await writeToFile(dirName + '/index.module.less', css);
      break;
    }
    default:
      break;
