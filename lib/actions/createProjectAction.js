...
const { IsCover, SelectFrameork, IsConfigEslint, IsConfigPrettier, IsConfigGitHooks, IsConfigTs } = require('../utils/prompts') //导入一些选项
...
const { ReactEslintDependencies, ReactPrettierDependencies, ReactTsDependencies, ReactGitHooksDependencies } = require('../config/react-config'); //导入eslint，prettier的相关package.json的依赖
...
const createProjectAction = async(projectName, options) => {
    ...
    let ReactPackage = await complie('react-package.ejs', { name: projectName.toLowerCase() })

    async function ConfigEslint() {
        if (isEslint) {
            let result = await complie('react-eslint.ejs', {})
                //向react项目中的package.json添加eslint的相关依赖
            ReactPackage = extendPackage(typeof ReactPackage === 'string' ? ReactPackage : JSON.stringify(ReactPackage), ReactEslintDependencies)
                //增加.eslintrc文件的目录
            let targetPath = path.resolve(`${process.cwd()+'/'+projectName}`, '.eslintrc')
    ...
    async function ConfigPrettier() {
        if (isPrettier) {
            ...
            ReactPackage = extendPackage(typeof ReactPackage === 'string' ? ReactPackage : JSON.stringify(ReactPackage), ReactPrettierDependencies)
                //增加.prettierrc文件的目录
            let targetPath = path.resolve(`${process.cwd()+'/'+projectName}`, '.prettierrc')
    ...
    async function ConfigTs() {
        if (isTs) {
            ...
            ReactPackage = extendPackage(typeof ReactPackage === 'string' ? ReactPackage : JSON.stringify(ReactPackage), ReactTsDependencies)
                //增加tsconfig.json文件的目录路径
            let tsConfigtPath = path.resolve(`${process.cwd()+'/'+projectName}`, 'tsconfig.json')
                //增加tsconfig.node.json文件的目录路径
            let tsConfigtNodePath = path.resolve(`${process.cwd()+'/'+projectName}`, 'tsconfig.node.json')
                //增加vite.config.ts文件的目录路径
            let viteConfigTsPath = path.resolve(`${process.cwd()+'/'+projectName}`, 'vite.config.ts')
            ...
            let viteConfigJsPath = path.resolve(`${process.cwd()+'/'+projectName}`, 'vite.config.js')
    ...
    async function ConfigGitHooks() {
        if (isGitHooks) {
            ...
            let targetPath = path.resolve(`${process.cwd()+'/'+projectName}`, '.husky')
            let sourcePath = path.resolve(__dirname, '../config/.husky')
            await fs.copy(sourcePath, targetPath)
                //添加git hooks的相关package.json配置
            ReactPackage = extendPackage(typeof ReactPackage === 'string' ? ReactPackage : JSON.stringify(ReactPackage), ReactGitHooksDependencies)
        }
    }
    let Repo = null
    switch (framework) {
    ...
    await download(Repo, projectName, { clone: true }, async(err) => {
        ...
        await ConfigTs()
        await writeToFile(path.resolve(process.cwd(), projectName, 'package.json'), JSON.stringify(ReactPackage))
